...
pybind11::dict puf_eval_log(pybind11::object pufferl_obj) {
    ...
    Dict* env_out = create_dict(128);  // 128 to fit rubiks per-depth hist + chess per-bank entries
...
py::dict vec_log(VecEnv& ve) {
    Dict* out = create_dict(128);  // rubiks logs ~82 keys (per-depth hist depth_0..35/solved_0..35)
