...
#define STATE_REWARD_WEIGHT \
  0.01f  // Fixed, small reward for maintaining "desirable" states
...
static inline float calculate_perf(unsigned char max_tile) {
  // Reaching 65k -> 1.0, 32k -> 0.8, 16k -> 0.4, 8k -> 0.2, 4k -> 0.1, 2k ->
  // 0.05
...
typedef struct {
  ...
  float reward_scaler;     // Pufferlib clips rew from -1 to 1, adjust the
                           // resulting rew accordingly

  float endgame_env_prob;  // The prob of env being initialized as an
                           // endgame-only env
  bool is_endgame_env;
  float scaffolding_ratio;  // The ratio for "scaffolding" runs, in which higher
                            // blocks are spawned
  bool is_scaffolding_episode;
  bool use_heuristic_rewards;
  float snake_reward_weight;
  bool use_sparse_reward;  // Ignore all rewards and provide 1 for reaching 16k,
                           // 32k, 65k
...
// Precomputed pow(x, 1.5) lookup table for x in [0, 19] to avoid expensive
// pow() calls.
...
void init(Game* game) {
  ...
  memset(game->seen, false, SIZE * SIZE * sizeof(bool));
...
void add_log(Game* game) {
  ...
  for (int i = 0; i < SIZE; i++) {
    for (int j = 0; j < SIZE; j++) {
      unsigned char tile = game->grid[i][j];
      switch (tile) {
        case 16:
          game->log.count_65536 += 1.0f;
        case 15:
          game->log.count_32768 += 1.0f;
        case 14:
          game->log.count_16384 += 1.0f;
        case 13:
          game->log.count_8192 += 1.0f;
        case 12:
          game->log.count_4096 += 1.0f;
        case 11:
          game->log.count_2048 += 1.0f;
        case 10:
          game->log.count_1024 += 1.0f;
        case 9:
          game->log.count_512 += 1.0f;
        case 8:
          game->log.count_256 += 1.0f;
      }
    }
  }

  switch (game->max_tile) {
    case 16:
      game->log.reached_65536 += 1.0f;
    case 15:
      game->log.reached_32768 += 1.0f;
      game->seen[15] = true;
    case 14:
      game->log.reached_16384 += 1.0f;
      game->seen[14] = true;
    case 13:
      game->log.reached_8192 += 1.0f;
      game->seen[13] = true;
    case 12:
      game->log.reached_4096 += 1.0f;
      game->seen[12] = true;
    case 11:
      game->log.reached_2048 += 1.0f;
      game->seen[11] = true;
    case 10:
      game->log.reached_1024 += 1.0f;
      game->seen[10] = true;
    case 9:
      game->log.reached_512 += 1.0f;
      game->seen[9] = true;
    case 8:
      game->log.reached_256 += 1.0f;
      game->seen[8] = true;
    default:
      break;
  }
  ...
  game->log.monotonicity_reward += game->monotonicity_reward *
                                   MONOTONICITY_REWARD_WEIGHT *
                                   game->reward_scaler;
  game->log.snake_reward +=
      game->snake_reward * game->snake_reward_weight * game->reward_scaler;
  game->log.n += 1;
}
...
void set_scaffolding_curriculum(Game* game) {
  ...
  if (game->lifetime_max_tile < 14) {
    ...
    if (curriculum < 2) {  // curriculum 0, 1
      place_tile_at_random_cell(
          game, 14 + curriculum);  // Spawn one of 16384 or 32768

    } else if (curriculum == 2) {
      // Place the tiles in the second row, so that they can be moved up in the
      // first move
...
void set_endgame_curriculum(Game* game) {
  ...
  // Place the tiles in the second-third rows, so that they can be moved up in
  // the first move
...
void c_reset(Game* game) {
  ...
  if (false) {
    if (game->is_endgame_env) {
      ...
      // Having high tiles saves moves to get there, allowing agents to
      // experience it faster
      game->is_scaffolding_episode =
          (rand() / (float)RAND_MAX) < game->scaffolding_ratio;
  ...
  // random chance  of choosing to take a curriculum tile.
  if (((float)rand()) / RAND_MAX < 0.33) {
    // find min and max seen
    ...
    if (max_seen != 0) {
      // game->grid[rand() % 4][rand() % 4] = max_seen;
    }
  }
...
static inline bool slide_and_merge(Game* game, unsigned char* row,
                                   float* reward, float* score_increase) {
  bool moved = false;
  int write_pos = 0;
...
float update_stats_and_get_heuristic_rewards(Game* game) {
  ...
  // Corner reward: A simple nudge to keep the max tiles horizontally in the top
  // row, left corner. When agents learn to put the max tile on the other
  // corners, or put max tiles vertically they miss out snake rew, and this does
  // happen sometimes.
  if (max_in_top_left && game->grid[0][1] == second_max_tile && max_tile > 4) {
    heuristic_state_reward += STATE_REWARD_WEIGHT;
  }

  // Snake reward: look for the snake pattern, only when the max tile is at top
  // left
  if (max_in_top_left) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      for (int j = 0; j < SIZE; j++) {
        ...
        // Vertical monotonicity: give score after row scanning for min/max is
        // done
        if (val != EMPTY && val < row_min) row_min = val;
        unsigned char next_row = game->grid[i + 1][j];
        if (next_row != EMPTY && next_row > next_row_max)
          next_row_max = next_row;
        // // Small column-level vertical reward
        if (val != EMPTY && next_row != EMPTY && val > next_row)
          monotonicity_reward += next_row;
      }
    ...
    // Snake bonus: sorted top row + the max_tile_in_row234 in the second row
    // right For example, top row: 14-13-12-11, second row: ()-()-()-10
  ...
  // return heuristic_state_reward + monotonicity_reward *
  // MONOTONICITY_REWARD_WEIGHT + snake_reward * game->snake_reward_weight; Max
...
void c_step(Game* game) {
  ...
  if (did_move) {
    ...
    // This is to limit infinite invalid moves during eval (happens for noob
    // agents) Don't need to be tight. Don't need to show to human player.
    int tick_multiplier =
        max(1, game->lifetime_max_tile -
                   8);  // practically no limit for competent agent
    game->max_episode_ticks =
        max(BASE_MAX_TICKS * tick_multiplier, game->score / 4);
  ...
  game->terminals[0] =
      (game_over || max_ticks_reached || max_level_reached) ? 1 : 0;
    ...
    for (int j = 0; j < SIZE; j++) {
      ...
      if (val > 0) {
        ...
        if (display_val < 10)
          ...
          x_offset = 25;  // 3-digit
        else if (display_val < 10000)
          x_offset = 15;  // 4-digit
        else if (display_val < 100000)
          x_offset = 2;  // 5-digit
        else {
          font_size = 24;
          x_offset = 5;
        ...
        DrawText(score_text, j * px + x_offset, i * px + 34, font_size,
                 PUFF_WHITE);
      }
    }
  }
